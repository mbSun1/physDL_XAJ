...
def parse_args():
    ...
    - 动态参数配置：虽然静态模式下不使用，但保持与训练脚本参数一致
    ...
    # =============================================================================
    # 动态参数配置参数（仅当td_opt=True时生效，静态参数模式下不使用）
    # =============================================================================
    # 这些参数控制动态参数模式下的特殊设置，在静态参数模式下不使用
    # 但保持与训练脚本参数一致，以确保模型正确加载
    # =============================================================================
...
def test_model_fold(args, rootDatabase, rootOut, puN, gageinfo, TestLS, TestInd, TrainLS, TrainInd, 
    ...
    # - exp_disp: 实验配置路径 (dPLXAJ/{puN}/Testforc/{for_type}/BuffOpt{buff_opt}/RMSE_para{alpha}/{random_seed}/Fold{test_fold}/{rnn_type})
    ...
    if args.pu_opt == 0: # ALL模式：时间泛化测试
        ...
        zTest = seriestest_norm[:, 0:TestBuff, :]  # 使用测试期前TestBuff天的数据作为反演输入
        xTest = np.concatenate([forcTestUN, PETTestUN], axis=2)  # 测试期 XAJ 强迫数据
        obs = obsTestUN[:, TestBuff:, :]  # 排除预热期的观测数据，从TestBuff天开始评估
...
def process_xaj_parameters(outpath, logtestIDLst, filePathLst):
    ...
    if xaj_params_files:
        ...
        print(f"- 站点数 (Number of basins): {combined_xaj_params.shape[0]}")
        print(f"- 参数数 (Number of parameters): {combined_xaj_params.shape[1]}")
        print(f"- 组件数 (Number of components): {combined_xaj_params.shape[2]}")
